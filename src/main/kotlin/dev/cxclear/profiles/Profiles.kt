...
private fun opencodeProtectedPaths(): List<Path> = buildList {
...
private fun dshProtectedPaths(): List<Path> = buildList {
    dshHome()?.let { root ->
        listOf(
            ".anonymous-user-id",
            ".credentials.yaml",
...
private fun dshHome(): Path? = homeSubdir(".dsh")

private fun dshDesktopAppData(): Path? =
    appDataRoaming()?.resolve("@deepseek-ai")?.resolve("dsh-desktop")?.takeIf { Files.isDirectory(it) }

private fun dshUpdaterCache(): Path? =
